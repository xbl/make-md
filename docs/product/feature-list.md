...
| 围栏代码语法高亮 | `complete` | 已支持带语言标记的代码块高亮，并补齐 `yaml` 等别名注册，修复了真实文件打开后 `ts`、`json`、`yaml` 等代码块高亮层被编辑器视图回收或降级的问题。 |
...
| 功能 | 状态 | 说明 |
| --- | --- | --- |
| 导出 HTML | `complete` | 已支持独立 HTML 导出，并补齐本地 Markdown 图片路径解析与图片 URL 预编码，避免打开导出后的 HTML 时相对图片失效或带空格图片无法识别。 |
